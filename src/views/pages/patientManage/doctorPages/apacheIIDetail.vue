  ...
  <div class="nursing-grading-detail">
    <!-- <div class="header">
      <div class="location">
        <el-breadcrumb separator="/">
          ...
          <el-breadcrumb-item class="location-other" :to="{ path: '/patient/list' }">患者详情</el-breadcrumb-item>
    ...
    <div class="main">
      ...
      <div class="title">
        ...
        <div style="float: right">
          <el-button type="primary" @click="add">新增评估</el-button>
        </div>
      </div>
      ...
      <el-table
        ...
        <div slot="empty" class="nodata">
          <img src="/static/img/nodata.png">
            ...
            <el-table-column prop="updateTs" label="评估时间" />
            <el-table-column prop="apsScore" label="APS评估结果" />
            <el-table-column prop="gcsSCore" label="GCS评估结果" />
            <el-table-column prop="chcScore" label="慢性健康评分" />
            <el-table-column prop="score" label="总分" />
            <el-table-column prop="realName" label="执行人" />
...
export default {
  ...
  methods: {
    ...
    getData() {
      ...
      this.loading = true
      this.$axios({
        url: global.url + '/score/queryApacheIIScoreList',
        method: 'post',
        data: params
      }).then((res) => {
    ...
    del(row) {
       this.$confirm("确认删除这条数据吗?", "提示", {
        ...
        this.$axios({
        url: global.url + '/score/deleteScoreRecord/' + row.answerId,
        method: 'post',
        data: {
    ...
    upd(row) {
      this.$refs.createDialog.init(this.formCode, 'update', this.patientRecordId, row.answerId)
    },
    initMap() {
      const data_time = []
      const data = []
